  ...
  //  $uce_opt3_query = $mysqli->query("INSERT INTO `uceresults` (`id`, `subject`, `result`, `student_studentID`) VALUES (NULL, '".$sopt3."', '".$gopt3."', '".$stdno."');");
   ...
   $x = 1;
    ...
    if ($studentbio_query && $uace_query && $uce_query) {
        ...
        mysqli_commit($mysqli);
        mysqli_close($mysqli);
        msg_success('Operation Successful', 'Update Successful');
...
/*check if regno no exists*/
function checkExists_RegNo($regNo) {
    include '../config/authModule/real-config.php';
...
function checkExists_stdNo($studentNo) {
    include '../config/authModule/real-config.php';
...
function insert_dep(){
    ...
    if (check_dep ($shortname) == false) {
        ...
        msg_error('Operation Failed', 'This Department Exists');
    }
}